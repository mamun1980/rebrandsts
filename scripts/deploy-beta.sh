...
docker build --no-cache -t re-brnad-sts-beta -f Dockerfile.beta .

### Complete Deployment
echo "killing the running docker"
docker ps -a | egrep 're-brnad-sts-beta' | awk '{print $1}'| xargs docker kill
docker ps -a | egrep 're-brnad-sts-beta' | awk '{print $1}'| xargs docker rm

echo "running the Travel Api using docker"
docker run -d --restart=unless-stopped --name re-brnad-sts-beta -p 9002:8000 re-brnad-sts-beta
