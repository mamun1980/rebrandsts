...
cd ../../
docker build --no-cache -t sts-$environment-$branch_name-$date -f Dockerfile.staging .
...
cd -
echo "Write the Image name to docker-compose-staging.yml & to a separate file for future reference"
sleep 2
sed -i "s|.sts:sts.*|/sts:sts-$environment-$branch_name-$date|g" docker-compose-staging.yml
